...
public class ChangePINForm extends JFrame implements IForm {
	...
	public ChangePINForm(ApplicationContext context, UserModel currentUser, AccountModel currentAccount) {
	...
	private void initUI() {
		...
		labContent1.setText("Xin vui lòng nhập mã PIN mới");
		...
		btnContinue.addActionListener(e -> actionForCon());
		btnCancel.addActionListener(e -> actionForCan());

	}

	// Ham xu ly su kien cho nut huy bo
	private void actionForCan() {
	...
	// Ham xu ly su kien cho nut tiep tuc
	private void actionForCon() {
		String newPIN = textPIN.getText();
		if (newPIN != null) {

			PINController pinController = context.getBean(PINController.class);

			try {
				currentAccount = pinController.changePIN(currentAccount, newPIN);
